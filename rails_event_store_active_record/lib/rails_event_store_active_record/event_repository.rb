...
module RailsEventStoreActiveRecord
  class EventRepository
    ...
    def append_to_stream(events, stream, expected_version)
      add_to_stream(Array(events), stream, expected_version) do |event|
    ...
    def link_to_stream(event_ids, stream, expected_version)
      ...
      (event_ids - Event.where(event_id: event_ids).pluck(:event_id)).each do |id|
        raise RubyEventStore::EventNotFound.new(id)
      end
      add_to_stream(event_ids, stream, expected_version) do |event_id|
        event_id
      end
    end
    ...
    def update_messages(messages)
      ...
      start_transaction do
        existing = Event.where(event_id: for_update).pluck(:event_id, :id).to_h
        (for_update - existing.keys).each{|id| raise RubyEventStore::EventNotFound.new(id) }
        hashes.each{|h| h[:id] = existing[h.fetch(:event_id)] }
